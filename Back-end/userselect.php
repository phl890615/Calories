...
if ((isset($_POST['modify'])) && ($_POST['modify'] == 'yes')) {
  ...
  $state = $_POST['state'];
  $gender = $_POST['gender'];
  $sql = "UPDATE member SET mem_name='$name', mem_email='$email', mem_bir='$bir', mem_state='$state' , mem_gender='$gender' where mem_phone = $phone ";
    ...
    <?php while ($row = mysqli_fetch_assoc($result)) { ?>
      <form method="post" name="form" style="text-align: center">
        <div class="input-group mb-3">
          ...
          <input type="text" name="email" value="<?php echo $row['mem_email']; ?>" class="form-control" aria-describedby="basic-addon1">
        </div>
        ...
        <div class="input-group mb-3">
          ...
          <input type="text" name="pwd" readonly="readyonly" value="<?php echo $row['mem_pwd']; ?>" class="form-control" aria-describedby="basic-addon1">
        </div>

        <div class="input-group mb-3">
          <span class="input-group-text" id="basic-addon1">會員暱稱:</span>
          <input type="text" name="name" value="<?php echo $row['mem_name']; ?>" class="form-control" aria-describedby="basic-addon1">
        </div>

        <div class="input-group mb-3">
          <span class="input-group-text" id="basic-addon1">會員性別:</span>
          <select name="gender" class="form-select" id="inputGroupSelect01">
            <option selected value="<?php echo $row['mem_gender'] ?>"> <?php if ($row['mem_gender'] == 0) {
                                                                          echo '男生';
                                                                        } else {
                                                                          echo '女生';
                                                                        }; ?> </option>
        ...
        <div class="input-group mb-3">
          ...
          <input type="text" name="bir" value="<?php echo $row['mem_bir']; ?>" class="form-control" aria-describedby="basic-addon1">
        </div>

        <div class="input-group" style="width: auto;">
          <span class="input-group-text">會員狀態:</span>
          <select name="state" class="form-select" id="inputGroupSelect01">
            <option selected value="<?php echo $row['mem_state'] ?>"> <?php if ($row['mem_state'] == 0) {
                                                                        echo '正常';
                                                                      } else {
                                                                        echo '封鎖';
                                                                      }; ?> </option>
