...
if(isset($_POST['diet_id'])!=''){
	...
	$data = "SELECT * FROM video where diet_plan_id = '$diet_id' order by video_id desc";
}else{
	...
	<div id="page">
       ...
       <hr>
				...
				<td>
					<form method="post" style='width: 700px;'>
						<div class="input-group mb-3">
							...
							<button class="btn btn-success" type="button" style="margin-left:20px" data-bs-toggle="collapse" data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
						...
						<div class="collapse" id="collapseExample">
							<div class="card card-body">
								<form action="" method="POST">
									<select style='font-size: 19px;' name="diet_id" class="form-control" onchange="submit();">
										<option select hidden value=""> 選擇飲食法 </option>
										<?php
		...
		<table class="" style="width: 700px; margin: 0 auto;padding: 0px;">
						...
						<tr>
							<td><?php echo "<video width='700' height='300'  controls><source src='../video/$rs[0].mp4?$time' type='video/mp4'></video>"; ?></td>
						</tr>
					</table>
					...
					<tr>
							...
							<td style="width: 378px;">
								<p style="font-size: 20px; padding-top: 0px;"><?php echo $co[1] ?></p>
							</td>
							<td>
								<p style="font-size: 20px;color:#8FBC8F; padding-top: 0px;"><?php echo $rs[15] ?></p>
							</td>
						</tr>
					</table>
					...
					<table style="width: 700px; margin: 0 auto;padding: 0px">
						<tr>
							<td>
								<p style="font-size: 25px;font-weight:bold;"><?php echo $rs[3] ?></p>
							</td>
						</tr>
					...
					<?php
						if ($rs[1] == $mem_phone) {
					?>
							<a href='video_revise.php?id=<?php echo $rs[0]; ?>' style='font-size: 18px; float: left;' class='btn btn-outline-success'>修改影片資料</a>
							<a href='video_delet.php?id= <?php echo $rs[0]; ?>' style='font-size: 18px; float: left;' class='btn btn-outline-success' onclick="return confirm('確認刪除')">刪除影片</a>
					<?php
						}
					?>
							</td>
						...
						<tr>
							<td>
								...
								<?php
							$like = "SELECT * FROM video_like where video_id = $rs[0] and mem_phone = $mem_phone";
							...
							if ($nums > 0) {
								echo "<button type='button' class='btn btn-danger' id='btn_collect<?php echo $i; ?>' value='$rs[0]'>$likenums<img src='../picture/heart.png' width='30'></button>";
							} else {
								echo "<button type='button' class='btn btn-light' id='btn_collect<?php echo $i; ?>' value='$rs[0]'>$likenums<img src='../picture/heart.png' width='30'></button>";
							}
						?>
  									<button type="button" class='btn btn-light'onclick="location.href='<?php echo "video_onemsg.php?id=" . $rs[0] ?>'"><?php echo $num; ?><img src="../picture/message.png" width="30"></button>
								</div>
							</td>
						</tr>
					</table>
