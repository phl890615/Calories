...
if (isset($_POST['search']) != '') {
	$search = $_POST['search'];
	$data = "SELECT * FROM video where video_title LIKE '%$search%' and mem_phone = '$mem_phone' order by video_id desc";
} else {
	$data = "SELECT * FROM video where mem_phone = '$mem_phone' order by video_id desc";
}
		...
		<table class="" style="width: 700px; margin: 0 auto;padding: 0px;">
				...
				<table class="" style="width: 700px; margin: 0 auto;padding: 0px;">
					...
					<?php
						if ($rs[1] == $mem_phone) {
					?>
							<a href='video_my_revise.php?id=<?php echo $rs[0]; ?>' style='font-size: 18px; float: left;' class='btn btn-outline-success'>修改影片資料</a>
							<a href='video_my_delet.php?id= <?php echo $rs[0]; ?>' style='font-size: 18px; float: left;' class='btn btn-outline-success' onclick="return confirm('確認刪除')">刪除影片</a>
					<?php
						}
					?>
				</td>
						...
						?>
							<button type="button" class='btn btn-light'onclick="location.href='<?php echo "video_my_onemsg.php?id=" . $rs[0] ?>'"><?php echo $num; ?><img src="../picture/message.png" width="30"></button>
					</div>
							</td>
						</tr>
					</table>
